 ...
 *
 * @warn
 *   Because the callback function is called via function pointer, the linker
 *   will generate an erroneous call tree. This should be fixed via the OVERLAY
 *   linker command line directive. The call tree must be rebuilt to indicate
 *   that the callback function is being called from the USBX_jumpCallback()
 *   function.
 *   For example, in the case of the callback function being called
 *   my_usbxp_callback() and the call to
 *   USBX_apiCallbackEnable(my_usbxp_callback) being made in the main()
 *   function of a file called main.c, this would look like:
 *
 *   OVERLAY(?PR?MAIN?USBXPRESS_ECHO_MAIN ~ ?PR?MY_USBXP_CALLBACK?USBXPRESS_ECHO_MAIN,
 *           ?PR?USBX_JUMPCALLBACK?EFM8_USBXPRESS ! ?PR?MY_USBXP_CALLBACK?USBXPRESS_ECHO_MAIN)
 *
